...
TimerHandle_t vctimer;
/* SysPm callback params */
cy_stc_syspm_callback_params_t callback_params = {.base = CYBSP_CSD_HW, .context = &cy_capsense_context};

cy_stc_syspm_callback_t capsense_deep_sleep_cb = {
    Cy_CapSense_DeepSleepCallback, CY_SYSPM_DEEPSLEEP, (CY_SYSPM_SKIP_CHECK_FAIL | CY_SYSPM_SKIP_BEFORE_TRANSITION | CY_SYSPM_SKIP_AFTER_TRANSITION), &callback_params, NULL, NULL};
...
void task_capsense(void *param) {
  ...
  scan_timer_handle = xTimerCreate("Scan Timer", CAPSENSE_SCAN_INTERVAL_MS, pdTRUE, NULL, capsense_timer_callback);
  ...
  vctimer = xTimerCreate("vcactivetime", VCTIMEOUT, pdFALSE, NULL, vcfalsestate);
...
static void process_touch(void) {
  ...
  if ((0u != button0_status) && (0u == button0_status_prev)) {
    vcsetstate();
    if (xTimerStartFromISR(vctimer, 0u) == pdFAIL)
      CY_ASSERT(0);
  }
...
static uint32_t capsense_init(void) {
  ...
  status = Cy_CapSense_RegisterCallback(CY_CAPSENSE_END_OF_SCAN_E, capsense_end_of_scan_callback, &cy_capsense_context);
  if (CYRET_SUCCESS != status) {
    return status;
  }
