...
#define GET_DUTY_CYCLE(x)                         \
  (100 - x) /* subtracting from 100 since the LED \
...
void task_led(void *param) {
  ...
  cyhal_pwm_set_duty_cycle(&pwm_led, GET_DUTY_CYCLE(LED_MAX_BRIGHTNESS), PWM_LED_FREQ_HZ);
  cyhal_pwm_start(&pwm_led);

  /* Repeatedly running part of the task */
  for (;;) {
    /* Block until a command has been received over queue */
    rtos_api_result = xQueueReceive(led_command_data_q, &led_cmd_data, portMAX_DELAY);
    ...
    if (rtos_api_result == pdTRUE) {
      ...
      case LED_UPDATE_BRIGHTNESS: {
        if ((led_on) || ((!led_on) && (led_cmd_data.brightness > 0))) {
          /* Start PWM to turn the LED on */
          cyhal_pwm_start(&pwm_led);
          uint32_t brightness = (led_cmd_data.brightness < LED_MIN_BRIGHTNESS) ? LED_MIN_BRIGHTNESS : led_cmd_data.brightness;
